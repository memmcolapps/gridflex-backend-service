...
//    })
    ...
    @Update({
            ...
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.meterManufacturer} THEN CAST(#{m.meterManufacturer} AS uuid)",
            "    </foreach>",
            "  END,",

            "  sim_number = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.simNumber} THEN #{m.simNumber} ",
            "    </foreach>",
            "  END,",

            "  meter_class = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.meterClass} THEN #{m.meterClass} ",
            "    </foreach>",
            "  END,",

            "  meter_number = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.meterNumber} THEN #{m.meterNumber} ",
            "    </foreach>",
            "  END,",

            "  old_sgc = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.oldSgc} THEN #{m.oldSgc} ",
            "    </foreach>",
            "  END,",

            "  new_sgc = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.newSgc} THEN #{m.newSgc} ",
            "    </foreach>",
            "  END,",

            "  old_krn = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.oldkrn} THEN #{m.oldkrn} ",
            "    </foreach>",
            "  END,",

            "  new_krn = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.newKrn} THEN #{m.newKrn} ",
            "    </foreach>",
            "  END,",

            "  old_tariff_index = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.oldTariffIndex} THEN #{m.oldTariffIndex} ",
            "    </foreach>",
            "  END,",

            "  new_tariff_index = CASE id",
            "    <foreach collection='batch' item='m'>",
            "      WHEN #{m.newTariffIndex} THEN #{m.newTariffIndex} ",
            "    </foreach>",
            "  END,",
